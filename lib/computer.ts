...
export class Computer {
  ...
  lastComp: BigNumber = new BigNumber(0);
  ...
  private computeBinary(node: AST.Binary) {
    ...
    if (node.operator === "=") {
      ...
      if (id == "_")
        throw new ComputerError(
          "Cannot write into read-only variable _",
          node.position.start,
          node.position.line
        );
      if (id == "__precision__")
        BigNumber.config({ DECIMAL_PLACES: lr.toNumber() });
      this.memory[id] = lr;
  ...
  private computeNode(node: AST.Node): BigNumber {
    switch (node.kind) {
      ...
      case AST.NodeKind.Identifier:
        ...
        throw new ComputerError(
          `Usage of uninitialized variable ${node.value}`,
          node.position.start,
          node.position.line
        );
